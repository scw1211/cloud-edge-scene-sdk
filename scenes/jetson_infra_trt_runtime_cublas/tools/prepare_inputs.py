#!/usr/bin/env python3
"""把 Infrared PNG 预处理成 NCHW float32 张量，供低延迟 TensorRT 路径直接读取。"""
import argparse
...
def prepare_one(source, destination, image_size):
    # 对齐 infra_module 的 torchvision/PIL 路径：
    # 转 RGB 三通道、PIL bicubic resize、ToTensor，再做 ImageNet mean/std 归一化。
...
def main():
    ...
    parser.add_argument("--input-dir", required=True, help="Infrared/test directory containing defect subdirectories")
