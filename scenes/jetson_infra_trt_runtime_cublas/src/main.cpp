...
constexpr float kBlurRadius = 4.0f;
...
void collect_pngs(const std::string& path, std::vector<std::string>* files) {
  // 原始图片路径：递归收集 Infrared/test 下所有 png，保持 defect 子目录结构。
...
std::vector<float> preprocess(const std::string& path, int width, int height) {
  // 非预处理模式直接在 C++ 内完成 OpenCV 解码、BGR->RGB 三通道、bicubic resize 和 ImageNet normalize。
...
int main(int argc, char** argv) {
  if (argc != 5 && argc != 7) {
    std::cerr << "Usage: infra_patchcore_trt ENGINE BANK INPUT_DIR OUTPUT_DIR [--preprocessed SOURCE_PNG_DIR]\n";
    return 2;
  }
  try {
    ...
    std::cout << "PATCHCORE_BACKEND=cuBLAS_GEMM\n";
    std::cout << "INPUT_SIZE=" << height << "x" << width << "\n";
    std::cout << "PATCH_GRID=" << grid << "x" << grid << " patches=" << patches << "\n";
    std::cout << "FEATURE_CHANNELS=" << channels << "\n";
    std::cout << "BANK_ROWS=" << bank.header.rows << " BANK_COLS=" << bank.header.cols << "\n";
    ...
    for (size_t i = 0; i < files.size(); ++i) {
      ...
      const double cold_start_to_result_ms = i == 0
          ...
          : 0.0;
      latency_csv << source_path << ',' << inference_ms << ',' << end_to_end_ms << ','<< cold_start_to_result_ms << "\n";
      latency_csv.flush();
    }
