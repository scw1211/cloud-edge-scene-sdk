...
RSS_LIMIT_KB="${RSS_LIMIT_KB:-1048576}"
E2E_LIMIT_MS="${E2E_LIMIT_MS:-100.0}"
COLD_START_LIMIT_MS="${COLD_START_LIMIT_MS:-5000.0}"
...
elif [ "${RSS_KB}" -gt "${RSS_LIMIT_KB}" ]; then
  echo "RSS gate failed: ${RSS_KB} KB > ${RSS_LIMIT_KB} KB"
  failures+=("RSS ${RSS_KB} KB > ${RSS_LIMIT_KB} KB")
else
  echo "RSS gate passed: ${RSS_KB} KB <= ${RSS_LIMIT_KB} KB"
fi
...
elif awk "BEGIN { exit !(${E2E_MAX_MS} >= ${E2E_LIMIT_MS}) }"; then
  echo "End-to-end latency gate failed: ${E2E_MAX_MS} ms >= ${E2E_LIMIT_MS} ms"
  failures+=("end-to-end latency ${E2E_MAX_MS} ms >= ${E2E_LIMIT_MS} ms")
else
  echo "End-to-end latency gate passed: ${E2E_MAX_MS} ms < ${E2E_LIMIT_MS} ms"
fi
...
elif awk "BEGIN { exit !(${COLD_START_MS} >= ${COLD_START_LIMIT_MS}) }"; then
  echo "Cold-start latency gate failed: ${COLD_START_MS} ms >= ${COLD_START_LIMIT_MS} ms"
  failures+=("cold-start latency ${COLD_START_MS} ms >= ${COLD_START_LIMIT_MS} ms")
else
  echo "Cold-start latency gate passed: ${COLD_START_MS} ms < ${COLD_START_LIMIT_MS} ms"
fi

# 指标门禁委托 Python verifier，和 base/Infra 中的权威基线按比例比较。
if "${PYTHON_BIN}" "${ROOT_DIR}/tools/verify_metrics.py" --predictions "${RESULT_DIR}" --base "${ROOT_DIR}/baseline/Infra" --class-name "${CLASS_NAME}" --img-size "${IMG_SIZE}" --output "${RESULT_DIR}/metrics.csv"; then
...
echo "Acceptance passed: RSS=${RSS_KB} KB, max end-to-end=${E2E_MAX_MS} ms, cold start=${COLD_START_MS} ms, Image AUROC > 85% baseline, Infra Pixel AUROC > 80% baseline, Infra Pixel F1 > 80% baseline"
