#!/usr/bin/env bash
set -euo pipefail
# 直接用红外 PNG 运行推理；会把解码/resize/normalize 放进 C++ 进程，主要用于功能验证。
...
"${ROOT_DIR}/build/infra_patchcore_trt" "${ENGINE_PATH}" "${BANK_PATH}" "${DATASET_PATH}/${CLASS_NAME}/Infrared/test" "${OUTPUT_DIR}"
