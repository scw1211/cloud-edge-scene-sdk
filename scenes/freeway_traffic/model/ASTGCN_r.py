...
class Spatial_Attention_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Jetson's PyTorch 2.1 ARM build can return NaN for the broadcast matmul
        # when the sigmoid tensor is non-contiguous and exactly zero.
...
class ASTGCN_submodule(nn.Module):
    ...
    def __init__(
        ...
        self.final_conv = nn.Conv2d(
            ...
            self.num_for_predict * self.output_dim,
    ...
    def encode(self, x):
        ...
        :return: shared spatio-temporal representation, (B, N_nodes, C, T)
        '''
        for block in self.BlockList:
            x = block(x)
        return x

    def decode_forecast(self, encoded):
        '''
        :param encoded: (B, N_nodes, C, T)
        :return: (B, N_nodes, T_out) when output_dim=1, otherwise (B, N_nodes, F_out, T_out)
        '''
        output = self.final_conv(encoded.permute(0, 3, 1, 2))[:, :, :, -1].permute(0, 2, 1)
        # (b,N,F,T)->(b,T,N,F)-conv<1,F>->(b,Fout*T,N,1)->(b,Fout*T,N)->(b,N,Fout*T)
    ...
    def forward(self, x):
        return self.decode_forecast(self.encode(x))
