#!/usr/bin/env bash
set -euo pipefail
# 正式低延迟推理入口：读取已预处理 .nchw.f32，并把原始 PNG 根目录传给 C++ 用于写回路径。
...
"${ROOT_DIR}/build/rgb_patchcore_trt" "${ROOT_DIR}/assets/vit_small_patch8_160_fp16.engine" "${BANK_PATH}" "${PREPROCESSED_DIR}" "${OUTPUT_DIR}" --preprocessed "${DATASET_PATH}/${CLASS_NAME}/RGB/test"
