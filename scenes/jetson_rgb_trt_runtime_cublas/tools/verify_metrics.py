#!/usr/bin/env python3
"""用 C++ 输出的原始预测图计算指标，并和权威 base/RGB 基线做门禁比较。"""
import argparse
import csv
...
def labels_for(image_path, img_size):
    ...
    if defect == "good":
        return 0, None
    csv_path = path.parents[2] / "GT" / defect / "data.csv"
    with csv_path.open() as handle:
        rows = list(csv.DictReader(handle))
    row = rows[int(path.stem)]
    # RGB baseline 的图像标签采用 any = RGB or infrared or pointcloud；像素指标只评估 RGB mask。
    image_label = int(int(row["RGB"]) or int(row["infrared"]) or int(row["pointcloud"]))
    if int(row["RGB"]) == 0:
        return image_label, None
    mask_path = path.parents[2] / "GT" / defect / path.name
...
def main():
    ...
    with open(Path(args.predictions) / "predictions.csv") as handle:
        for row in csv.DictReader(handle):
            # predictions.csv 中 map_file 指向 C++ 写出的 160x160 float32 原始得分图。
    ...
    actual = {"Image_ROCAUC": roc_auc_score(image_labels, image_scores), "RGB_Pixel_ROCAUC": roc_auc_score(pixel_labels, pixel_scores), "RGB_Pixel_F1": float(f1.max()), "RGB_Pixel_AUPR": auc(recall, precision), "RGB_Pixel_AP": average_precision_score(pixel_labels, pixel_scores)}
    with open(args.base) as handle:
        target = next(row for row in csv.DictReader(handle) if row["Method"] == "RGB" and row["Class"].lower() == args.class_name)
    required_ratios = {
        # 正式部署门禁：Image AUROC 要求大于原版 baseline 的 90%；
        # RGB Pixel AUROC 和 RGB Pixel F1 都要求大于原版 baseline 的 80%。
        "Image_ROCAUC": 0.90,
        "RGB_Pixel_ROCAUC": 0.80,
        "RGB_Pixel_F1": 0.80,
    }
    metric_columns = ["Method", "Image_ROCAUC", "RGB_Pixel_ROCAUC", "RGB_Pixel_F1", "RGB_Pixel_AUPR", "RGB_Pixel_AP", "Class"]
    metric_row = {
        "Method": "RGB",
        "Image_ROCAUC": round(actual["Image_ROCAUC"], 3),
        "RGB_Pixel_ROCAUC": round(actual["RGB_Pixel_ROCAUC"], 3),
        "RGB_Pixel_F1": round(actual["RGB_Pixel_F1"], 3),
        "RGB_Pixel_AUPR": round(actual["RGB_Pixel_AUPR"], 3),
        "RGB_Pixel_AP": round(actual["RGB_Pixel_AP"], 3),
