# Joint static Q5 candidate v2

This namespace fixes the package input contract to the canonical
`scene-prefixed-decimal17@v1` encoder. It is isolated from both the formal
release store and the rejected v1 sidecar evidence.
...
REGISTRY=$ROOT/runtime/traffic_industrial_joint_static_q5km_candidate_v2/edge_llm_release_store.json
...
python3 -m edge_llm_factory serve-release \
  ...
  --port 19393 \
  --runtime-output "$ROOT/scenes/industrial_anomaly/deployment/joint_static_candidate_v2/runtime_output_traffic.json" \
  --runtime-output "$ROOT/scenes/industrial_anomaly/deployment/joint_static_candidate_v2/runtime_output_industrial.json" \
  --startup-probe-config "$ROOT/scenes/industrial_anomaly/deployment/joint_static_candidate_v2/startup_probes.json" \
  --print-command
```
