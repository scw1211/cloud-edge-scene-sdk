...
- [ ] 需要强制云复核的局部异常已映射为 `cloud_review_requested`，并声明 `minimum_evidence_level`
...
- [ ] 恢复重放后边缘与云端反馈样本库均新增记录，且保留边缘初判和云端修正
- [ ] `/metrics` 保存了闭环时延、通信字节、路由和自治统计
- [ ] 需要多边缘汇聚时，插件已定义关联键、成员、最小成员数和超时策略
- [ ] 大证据文件已按SHA-256上传并验证去重，未把估算字节冒充实际通信量
- [ ] 已保存边缘临时判断、云端最终结果、修正率和异步最终时延
- [ ] 校准、覆盖率或漂移监测失效时能够触发强制复核
- [ ] 学习式路由在真实反馈不足时保持影子模式，不直接接管安全规则
