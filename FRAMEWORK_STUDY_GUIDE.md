...
本文对应独立SDK `cloud-edge-scene-sdk` 0.13.1和公共框架0.4.1。目标不是教你逐行背代码，而是让你能回答三件事：

1. 一个场景事件从边缘进入后，经过哪些对象和模块；
2. 云断开、请求重复、模型更新或多节点冲突时，系统如何处理；
3. 新场景应该改哪些文件，哪些公共文件不应修改。

如果要准备项目汇报、答辩或向其他成员完整介绍所有模块，请配合阅读`docs/系统全模块介绍与答辩讲解.md`。
...
上述是公共调度器的通用路由。场景若声明了多边缘汇聚规格，每个样本的轻量摘要都必须让汇聚端可见。普通业务先把摘要持久化到 Outbox，立即返回`provisional`，再由后台发送；明确要求完整云确认且预算允许时保留`cloud_sync`，若其他成员尚未到齐则返回临时结果并自动转入 Outbox。摘要是否上报与业务是否等待是两个独立问题。
...
场景感知模型
  ...
  -> CollaborationScheduler.schedule()
       ...
       -> 有 aggregation_spec -> 强制先写 Outbox，立即返回 provisional
  -> OutboxReplayWorker 后台调用云端 aggregate()/coordinate()
...
正常网络下，边缘始终先给出本地临时结果。普通监测样本的摘要通过持久 Outbox 后台发送，不阻塞业务；明确要求完整云确认且闭环预算允许的动作可以同步等待。断网自治事件只要已经进入补传队列，全局状态就是“待复核”，不能因为当前已经返回一个安全动作就冒充全局最终结果。需要云确认的不可逆动作只进入延后列表，不会提前授权。

生命周期分别统计边缘临时结果、云端首次接收、轻量最终、大模型复核和大模型修正。`cloud_receipt`是一程跨机器时间，必须在边缘与云端使用同一 NTP/chrony 时间源且记录时钟偏差后才能用于正式报告；HTTP 往返时间由边缘单机单调时钟测量，不依赖跨机时钟同步。

## 五、多节点协调如何工作

`CloudRuntime.coordinate()` 先按场景调用 `fuse_cloud_context()`，场景插件只能补充上下文，不能改变事件数量和身份。之后云端分别生成候选决策，再交给 `ConflictCoordinator`。

在线到达的多边缘事件还可以通过`aggregation.py`按场景插件提供的关联键持久汇聚。全部预期成员到齐时立即协调；超时但满足最小成员数时执行部分汇聚；重复事件按事件编号去重。部分结果标记为 `partial_final` 和 `evidence_complete=false`，只能作为临时修正，不能授权要求完整云确认的动作。迟到成员会重开该组并增加结果版本；即使成员恰好在云端计算期间到达，也会强制重新汇聚，避免把已经 4/4 的组永久冻结成 2/4 结果。边缘在云端首次接收后使用短轮询等待完整结果，得到部分结果后改用慢轮询，并受 `aggregation_max_wait_seconds` 限制；到期后普通 Outbox 结束，生命周期明确记录为 `partial_final` 或 `local_only_timeout`。随后同一 SQLite 中的有界低频 reconciliation 子队列按 `reconciliation_poll_seconds` 间隔、只在 `reconciliation_max_wait_seconds` 窗口内查询迟到版本：若完整成员到齐，就把原生命周期升级为权威 final；否则到期自动清除，因此不会让 Outbox 无限增长。关联键、成员名称、预期成员和超时时间均由场景插件定义，公共框架不会假设所有场景都使用`sample_id`。
...
需要使用边缘文本大模型的场景，可在共享文本基座上独立训练 LoRA；这是可选的场景能力，不是公共框架对所有场景的强制要求：
...
4. 为什么多边缘摘要始终后台上报，而业务是否等待最终结果要单独决定？
